...
export default function App() {
  ...
  }, [currentGuess, guesses, gameStatus, solution, mode, timer, stats]);

  const saveDaily = (finalGuesses: string[], status: string, finalTime: number) => {
    const today = new Date().toISOString().split('T')[0];
    localStorage.setItem(`daily-${today}`, JSON.stringify({ guesses: finalGuesses, solution, status, time: finalTime }));
  };
  ...
  return (
    <div className="flex flex-col items-center min-h-screen bg-white text-slate-900 p-2 font-sans select-none overflow-x-hidden">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-white/95 flex items-center justify-center p-6 z-50">
          <div className="bg-white border-2 border-black p-8 rounded-2xl shadow-2xl text-center max-w-xs w-full relative">
            ...
            <h2 className="text-2xl font-black mb-1">{gameStatus === 'won' ? 'SOLVED!' : 'WORD WAS'}</h2>
            <p className="text-3xl font-black text-green-600 mb-4">{solution}</p>
            
            <div className="bg-gray-50 p-4 rounded-xl mb-6 text-sm text-left italic text-gray-600 border-l-4 border-blue-500">
                "{definition || 'Loading definition...'}"
            </div>
            
            <div className="flex flex-col gap-3">
              <button onClick={shareResults} className="w-full bg-blue-600 text-white py-3 rounded-xl font-bold active:scale-95 transition">Share SL Wordle</button>
              {mode === 'practice' && <button onClick={resetGameState} className="w-full bg-black text-white py-3 rounded-xl font-bold">Next Practice</button>}
            </div>
          </div>
        </div>
      )}
      ...
      {showStats && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-6 z-[60]">
            <div className="bg-white p-8 rounded-3xl w-full max-w-sm text-center">
                <h2 className="text-2xl font-black mb-6">CAREER STATS</h2>
                <div className="grid grid-cols-2 gap-4 mb-8">
                    <div className="bg-gray-100 p-4 rounded-2xl">
                        <p className="text-2xl font-black">{stats.gamesPlayed}</p>
                        <p className="text-[10px] font-bold text-gray-500 uppercase">Played</p>
                    </div>
                    <div className="bg-gray-100 p-4 rounded-2xl">
                    ...
                    <div className="bg-gray-100 p-4 rounded-2xl">
                        <p className="text-2xl font-black">{stats.gamesWon > 0 ? Math.round(stats.totalTime / stats.gamesWon) : 0}s</p>
                        <p className="text-[10px] font-bold text-gray-500 uppercase">Avg Speed</p>
                    </div>
                    <div className="bg-gray-100 p-4 rounded-2xl">
                        <p className="text-2xl font-black">{stats.currentStreak}</p>
                        <p className="text-[10px] font-bold text-gray-500 uppercase">Streak</p>
                    </div>
                </div>
      ...
      {message && <div className="fixed top-8 left-1/2 -translate-x-1/2 bg-black text-white px-6 py-3 rounded-full font-bold shadow-2xl z-[70] animate-bounce">{message}</div>}
    </div>
  );
}
...
function Row({ guess, isSubmitted, solution }: { guess: string, isSubmitted: boolean, solution: string }) {
  ...
  const colors = isSubmitted ? getRowColors(guess, solution) : Array(5).fill('');
  ...
  return (
    <div className="flex gap-1.5">
      {letters.map((char, i) => {
        ...
        if (isSubmitted) {
            if (colors[i] === 'green') bgColor = "bg-green-600 border-green-600 text-white";
            else if (colors[i] === 'yellow') bgColor = "bg-yellow-500 border-yellow-500 text-white";
            else bgColor = "bg-gray-500 border-gray-500 text-white";
        } else if (char !== ' ') bgColor = "border-gray-600 scale-105";

        return (
          <div key={i} className={`w-12 h-12 sm:w-14 sm:h-14 border-2 flex items-center justify-center text-2xl sm:text-3xl font-black transition-all duration-500 ${bgColor}`}>
            {char.trim()}
          </div>
        );
      })}
